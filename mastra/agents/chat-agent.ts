...
const instructions = [
  ...
  "Changing an existing workflow works the same way: `create_workflow` under the same id replaces what's stored. Read the current definition with `get_workflow` first unless you wrote it earlier in this conversation, and send the complete edited definition — every entry you're keeping, not just the changed ones.",
  "For anything else, just answer clearly and concisely. Use markdown when it makes the answer easier to read.",
].join("\n\n")

/**
 * The workflow the user has open on the canvas, sent with every message (see
 * `app/api/chat/route.ts`). Without it, "add a step to this one" in a fresh
 * conversation has nothing to point at — the definition was built in a chat
 * that's gone, so the history doesn't hold it and neither does the model.
 */
function openWorkflowNote(id: unknown) {
  if (typeof id !== "string" || !id) {
    return []
  }

  return [
    `The user currently has the workflow \`${id}\` open on the canvas. When they say "this workflow", "it", or ask for a change without naming one, that is the workflow they mean — call \`get_workflow\` with that id to read what it does now rather than assuming. Give a workflow a new id only when the user clearly wants a separate one alongside it.`,
  ]
}
...
export const chatAgent = new Agent({
  ...
  instructions: ({ requestContext }) =>
    [
      instructions,
      ...openWorkflowNote(requestContext.get("openWorkflowId")),
    ].join("\n\n"),
  ...
  // Writing a definition and reading one back. The triage blocks are
  // deliberately not here: the agent writes workflows that reference them, it
  // doesn't call them.
  tools: { create_workflow: createWorkflow, get_workflow: getWorkflow },
